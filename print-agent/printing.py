...
def print_pdf(pdf_path: str, printer_name: str, rotation_degrees: int = 0, fit_to_page: bool = False) -> None:
    """
    ...
    `fit_to_page` uses SumatraPDF's own "-print-settings fit" option, which
    scales the PDF's content to match whatever paper size the printer is
    currently set to. Used for the hot-folder flow (arbitrary label PDFs
    downloaded from eBay/the carrier's own site), where we have no prior
    knowledge of the PDF's page size - unlike dashboard-triggered jobs,
    which are generated by this system and already sized correctly.
    """
    ...
    args = [SUMATRA_PATH, "-print-to", printer_name, "-silent", "-exit-when-done"]
    if fit_to_page:
        args += ["-print-settings", "fit"]
